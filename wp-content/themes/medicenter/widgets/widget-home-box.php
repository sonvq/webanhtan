...
class home_box_widget extends WP_Widget 
{
    ...
    function widget($args, $instance) 
	{
		...
		echo $before_widget;
		?>
		<li class="home-box-container clearfix<?php echo ($animation!='' ? ' animated-element animation-' . $animation . ((int)$animation_duration>0 && (int)$animation_duration!=600 ? ' duration-' . (int)$animation_duration : '') . ((int)$animation_delay>0 ? ' delay-' . (int)$animation_delay : '') : ''); ?>"<?php echo ($custom_color!="" || (int)$width>0 ? ' style="' . ($custom_color!="" ? 'background-color: #' . $custom_color . ';' : '') . ((int)$width>0!="" ? ($custom_color!="" ? ' ' : '') . 'width: ' . $width . 'px;' : '') . '"' : ''); ?>>
			<div class="home-box">
				<?php
				if($title) 
				{
				...
				<div class="news clearfix">
					...
					echo '<a' . (!empty($button_style) && $button_style=="custom" ? ($button_color!="" || $button_text_color!="" ? ' style="' . ($button_color!="" ? 'background-color:' . $button_color . ';border-color:' . $button_color . ';' : '') . ($button_text_color!="" ? 'color:' . $button_text_color . ';': '') . '"' : '') . ($button_hover_color!="" || $button_hover_text_color!="" ? ' onMouseOver="' . ($button_hover_color!="" ? 'this.style.backgroundColor=\''.$button_hover_color.'\';this.style.borderColor=\''.$button_hover_color.'\';' : '' ) . ($button_hover_text_color!="" ? 'this.style.color=\''.$button_hover_text_color.'\';' : '' ) . '" onMouseOut="' . ($button_hover_color!="" ? 'this.style.backgroundColor=\''.$button_color.'\';this.style.borderColor=\''.$button_color.'\';' : '' ) . ($button_hover_text_color!="" ? 'this.style.color=\''.$button_text_color.'\';' : '') . '"' : '') : '') . ' title="'.esc_attr($title).'" href="'.esc_url($href).'"'.$target.' class="mc-button more template-arrow-horizontal-1-after' . (!empty($button_style) && $button_style!="custom" ? ' ' . esc_attr($button_style) : '') . '">'.$title.'</a>';
					endif; ?>
				</div>
			</div>
		</li>
