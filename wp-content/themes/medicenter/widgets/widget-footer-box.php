...
class footer_box_widget extends WP_Widget 
{
    ...
    function widget($args, $instance) 
	{
		...
		echo $before_widget;
		?>
		<li class="footer-banner-box<?php echo ($icon!='' ? ' features-' . esc_attr($icon) : '') . ($animation!='' ? ' animated-element animation-' . esc_attr($animation) . ((int)$animation_duration>0 && (int)$animation_duration!=600 ? ' duration-' . (int)$animation_duration : '') . ((int)$animation_delay>0 ? ' delay-' . (int)$animation_delay : '') : ''); ?>"<?php echo ($custom_color!="" ? ' style="background-color: #' . esc_attr($custom_color) . '"' : ''); ?>>
			<?php
			if($title) 
			{
				if($title_color!="")
			...
			?>
			<?php if($content!="" || $url!=""):	?>
			<p<?php echo ($icon!='' ? ' class="content-margin"' : '') . ($text_color!="" ? " style='color: #" . $text_color . ";'" : ""); ?>>	
			<?php 
			if($content!="")
	...
	{	
		...
		<p>
			<label for="<?php echo $this->get_field_id('icon'); ?>"><?php _e('Icon', 'medicenter'); ?></label>
			<select id="<?php echo $this->get_field_id('icon'); ?>" name="<?php echo $this->get_field_name('icon'); ?>">
				<option value="">-</option>
				<?php for($i=0; $i<count($icons); $i++)
				{
				?>
				<option value="<?php echo $icons[$i]; ?>"<?php echo (isset($icon) && $icons[$i]==$icon ? " selected='selected'" : "") ?>><?php echo $icons[$i]; ?></option>
				<?php
				}
				?>
			</select>
