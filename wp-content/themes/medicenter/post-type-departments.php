...
function theme_inner_departments_custom_box_side($post) 
{
	...
	<ul id="department_hours_list"' . (!$department_hours_count ? ' style="display: none;"' : '') . '>';
		for($i=0; $i<$department_hours_count; $i++)
		{
			...
			echo '<li id="department_hours_' . $department_hours[$i]->department_hours_id . '">' . $current_day->post_title . ' ' . date("H:i", strtotime($department_hours[$i]->start)) . '-' . date("H:i", strtotime($department_hours[$i]->end)) . '<img class="operation_button delete_button" src="' . get_template_directory_uri() . '/admin/images/delete.png" alt="del" /><img class="operation_button edit_button" src="' . get_template_directory_uri() . '/admin/images/edit.png" alt="edit" /><img class="operation_button edit_hour_department_loader" src="' . get_template_directory_uri() . '/admin/images/ajax-loader.gif" alt="loader" />';
			$doctorsString = "";
