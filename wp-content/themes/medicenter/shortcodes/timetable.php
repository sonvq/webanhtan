...
function theme_timetable($atts, $content)
{
	...
	{
		$output .= '<ul class="clearfix tabs-box-navigation sf-menu' . ($top_margin!="none" ? ' ' . $top_margin : '') . '">
	...
	$output .= '<div id="all-departments">' . get_timetable($departments_page, $departments_array, $mode, $hour_category, $tip_block) . '</div>';
	for($i=0; $i<$departments_array_count; $i++)
		$output .= '<div id="' . $departments_array[$i] . '">' . get_timetable($departments_page, $departments_array[$i], $mode, $hour_category, $tip_block) . '</div>';
	$output .= '</div>';
...
function get_row_content($departments, $departments_page, $mode)
{
	...
	foreach($departments as $key=>$details)
	{
		...
		$class_link = '<a class="event-header" href="' . $classes_url . '#' . urldecode($details["name"]) . '" title="' .  esc_attr($key) . '"' . ($text_color!="" ? ' style="color: #' . $text_color . ';"' : '') . '>' . $key . '</a>';
		$content .= $class_link;
		...
		for($i=0; $i<$hours_count; $i++)
		{
			...
			$content .= '<div class="hours"' . ($hours_text_color!="" ? ' style="color:#' . $hours_text_color . ';"' : '') . '>' . $details["hours"][$i] . '</div>';
		...
		{
			$hover_color = get_post_meta($details["id"], $themename . "_hover_color", true);
			$content .= '<div class="tooltip-text"><div class="tooltip-content"' . ($hover_color!="" || $hover_text_color!="" ? ' style="' . ($hover_color!="" ? 'background-color: #' . $hover_color . ';' : '') . ($hover_text_color!="" ? 'color: #' . $hover_text_color . ';' : '') . '"': '') . '>' . $tooltip . '</div><span class="tooltip-arrow"' . ($hover_color!="" ? ' style="border-color: #' . $hover_color . ' transparent;"' : '') . '></span></div>';	
		}
		if(count($departments)>1)
			$content .= '</div>';
	}
...
function get_timetable($departments_page, $department = null, $mode = null, $hour_category = null, $tip_block = 1)
{
	global $themename;
	global $blog_id;
	global $wpdb;
