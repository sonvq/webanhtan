...
$output .= '<a' . (!empty($style) && $style=="custom" ? ' style="color:'.$text_color.';background-color:'.$color.';border-color:'.($color=="transparent" ? '#E5E5E5' : $color).';" onMouseOver="this.style.color=\''.$hover_text_color.'\';this.style.backgroundColor=\''.$hover_color.'\';this.style.borderColor=\''.($hover_color=="transparent" ? '#E0E0E0' : $hover_color).'\'" onMouseOut="this.style.color=\''.$text_color.'\';this.style.backgroundColor=\''.$color.'\';this.style.borderColor=\''.($color=="transparent" ? '#E0E0E0' : $color).'\'"' : '') . ' title="'.$title.'" href="'.$href.'"'.$target.' class="mc-button more'.(!empty($style) && $style!="custom" ? ' ' . esc_attr($style) : '').$size.$icon.$el_class.$position.$a_class.($top_margin!='none' ? ' ' . $top_margin : '').'">'.$title.(substr($icon_orig, 0, 8)!="template" ? $i_icon : '').'</a>';
